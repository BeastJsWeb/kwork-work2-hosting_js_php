           ...
           foreach ($comments as $key => $row)
           {
             ...
             <div id="comment_id_<?php echo $row['id']; ?>" class="comment <?php echo $level_up > 1 ? 'comment--reply' : ''; ?>" data-next="<?php echo $arr_next; ?>" data-image="<?php echo !empty($row['image']) ? $row['image'] : ''; ?>" data-id="<?php echo $row['id']; ?>" data-level="<?php echo $level + 1; ?>" style="--level: <?php echo $level + 1; ?>;">
              ...
              <div class="comment__content">
               ...
               <div class="comment__icon" style="cursor: pointer;" onclick="form_complaint_id.value = '<?php echo $row['id']; ?>'; form_complaint.reset(); siteModal = new bootstrap.Modal(document.getElementById('modal_complaint')); siteModal.show(); return false;" title="Пожаловаться">
               ...
               if (isset($_SESSION['USER_ID']))
               {
                 if ($row['user_id'] == $_SESSION['USER_ID'] && time() - strtotime($row['time_add']) < 300)
                 {
                   ...
                   <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                    <li><a class="dropdown-item comment-edit" href="#"><svg height="20" width="20" class="icon icon--v_pen"><use xlink:href="#v_pen"></use></svg> Редактировать</a></li>
                    <li><a class="dropdown-item" href="#" onclick="ajax('delete.php?id=<?php echo $row['id']; ?>'); return false;"><svg height="20" width="20" class="icon icon--v_pen"><use xlink:href="#v_delete"></use></svg> Удалить</a></li>
                   </ul>
                   <?php
                 }
               }
     ...
     <div class="modal-body">
      <form id="form_register" novalidate="" autocomplete="off">
       ...
       <div class="d-flex align-items-center justify-content-between">
        <div>Есть аккаунт? <a href="#" class="link-info btns_login" onclick="siteModal.hide(); siteModal = new bootstrap.Modal(document.getElementById('modal_login')); siteModal.show(); return false;">Войти</a></div>
        <button class="btn btn-primary" onclick="ajax('reg.php', '#form_register'); return false;">Зарегистрироваться</button>
       </div>
      </form>
     </div>
    </div>
     ...
     <div class="modal-body">
      <form id="form_login" novalidate="" autocomplent="off">
       ...
       <div class="d-flex align-items-center justify-content-between">
        ...
        <button class="btn btn-primary" onclick="ajax('log.php', '#form_login'); return false;">Войти</button>
       </div>
      </form>
     </div>
    </div>
     ...
     <div class="modal-body">
      <form id="form_restore" novalidate="" autocomplent="off">
       ...
       <div class="d-flex align-items-center justify-content-between">
        ...
        <button class="btn btn-primary" onclick="ajax('log.php', '#form_restore'); return false;">Отправить письмо с паролем</button>
       </div>
      </form>
     </div>
    </div>
   ...
   <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
     ...
     <div class="modal-footer">
      <button type="button" class="btn btn-primary" onclick="ajax('complaint.php', '#form_complaint'); return false;">Отправить</button>
     </div>
    </div>
   </div>
  </div>
